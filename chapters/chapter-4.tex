...
Ringkasan konfigurasi \textit{ablation study} yang digunakan pada penelitian ini ditunjukkan pada Tabel~\ref{table:4.ablation-config}.

\begin{table}[htbp]
\centering
\caption{Konfigurasi \textit{Ablation Study}}
...
\midrule
BASE-SP & 79,2\% & 76,8\% & 76,7\% & 76,7\% & 86,8\% \\
M1-SP & 78,0\% & 75,8\% & \textbf{77,5\%} & 76,3\% & 85,1\% \\
BASE-ST & \textbf{85,8\%} & \textbf{83,8\%} & \textbf{85,5\%} & \textbf{84,5\%} & \textbf{92,3\%} \\
...
\midrule
BASE-SP & 79,2\% & 76,8\% & 76,7\% & 76,7\% & 86,8\% \\
M3-SP & \textbf{81,8\%} & \textbf{79,7\%} & \textbf{81,3\%} & \textbf{80,3\%} & \textbf{88,1\%} \\
BASE-ST & \textbf{85,8\%} & \textbf{83,8\%} & \textbf{85,5\%} & \textbf{84,5\%} & \textbf{92,3\%} \\
...
Hasil pengaruh augmentasi data terhadap performa model ditunjukkan pada Tabel~\ref{table:4.ablation-augment}.
...
\midrule
BASE-SP & 79,2\% & 76,8\% & 76,7\% & 76,7\% & 86,8\% \\
M5-SP & \textbf{79,4\%} & \textbf{77,0\%} & \textbf{77,9\%} & \textbf{77,4\%} & \textbf{87,1\%} \\
BASE-ST & \textbf{85,8\%} & \textbf{83,8\%} & \textbf{85,5\%} & \textbf{84,5\%} & \textbf{92,3\%} \\
...
Hasil pengaruh \textit{scheduler} terhadap performa model ditunjukkan pada Tabel~\ref{table:4.ablation-scheduler}.
...
\midrule
BASE-SP & 79,2\% & 76,8\% & 76,7\% & 76,7\% & 86,8\% \\
M6-SP & \textbf{82,5\%} & \textbf{80,4\%} & \textbf{80,7\%} & \textbf{80,6\%} & \textbf{88,7\%} \\
BASE-ST & \textbf{85,8\%} & \textbf{83,8\%} & \textbf{85,5\%} & \textbf{84,5\%} & \textbf{92,3\%} \\
...
Secara keseluruhan, hasil \textit{ablation study} menunjukkan bahwa setiap perubahan konfigurasi memberikan pengaruh yang berbeda pada model spasial dan spasiotemporal. Beberapa konfigurasi hanya meningkatkan salah satu pendekatan, sedangkan konfigurasi lainnya menurunkan performa pada kedua pendekatan. Pada tahap ini, \textit{scheduler} dipilih sebagai konfigurasi lanjutan karena memberikan hasil rata-rata yang paling baik untuk digunakan secara adil pada kedua model. Dengan demikian, eksperimen utama pada subbab berikutnya menggunakan konfigurasi \textit{scheduler} hasil \textit{ablation study} sebagai dasar pelatihan.

\subsection{Hasil Eksperimen Utama} \label{IV.Eksperimen_Utama}

Eksperimen utama dilakukan untuk mengevaluasi pengaruh konfigurasi input terhadap performa model spasial dan spasiotemporal. Berbeda dari \textit{ablation study} yang berfokus pada perubahan konfigurasi pelatihan, eksperimen utama berfokus pada perubahan karakteristik masukan, yaitu resolusi, jumlah \textit{frame}, dan \textit{stride}. Konfigurasi BASE telah digunakan dan dianalisis sebelumnya pada eksperimen \textit{baseline} sebagai konfigurasi dasar, yaitu resolusi 224 $\times$ 224, jumlah \textit{frame} masukan 16, dan \textit{stride} 3. Oleh karena itu, pada bagian ini BASE digunakan sebagai acuan konfigurasi, sedangkan eksperimen E2 sampai E5 digunakan untuk menganalisis pengaruh variasi input terhadap performa kedua pendekatan.

Konfigurasi eksperimen utama yang digunakan ditunjukkan pada Tabel~\ref{table:4.eksperimen-utama-config}.

\begin{table}[htbp]
\centering
\caption{Konfigurasi Eksperimen Utama}
...
Hasil eksperimen penurunan resolusi ditunjukkan pada Tabel~\ref{table:4.eksperimen-utama-resolusi}.
...
Hasil eksperimen \textit{stride} lebih rapat ditunjukkan pada Tabel~\ref{table:4.eksperimen-utama-stride}.
...
Hasil eksperimen \textit{frame} lebih sedikit ditunjukkan pada Tabel~\ref{table:4.eksperimen-utama-frame-sedikit}.
...
Hasil eksperimen \textit{frame} lebih banyak ditunjukkan pada Tabel~\ref{table:4.eksperimen-utama-frame-banyak}.
...
Pada evaluasi ini, \textit{checkpoint} model yang digunakan berasal dari hasil \textit{ablation study scheduler}, yaitu M6-SP untuk model spasial dan M6-ST untuk model spasiotemporal. Kedua \textit{checkpoint} tersebut dipilih karena konfigurasi \textit{scheduler} pada M6 telah ditetapkan sebagai konfigurasi lanjutan pada eksperimen sebelumnya. Hasil pengujian pada \textit{Celeb-DF v2} diberi kode E6-SP untuk model spasial dan E6-ST untuk model spasiotemporal. Evaluasi dilakukan terhadap 518 klip uji dari \textit{dataset} \textit{Celeb-DF v2}.

Hasil evaluasi lintas-\textit{dataset} pada \textit{dataset} \textit{Celeb-DF v2} ditunjukkan pada Tabel~\ref{table:4.cross-dataset}.
...
Berdasarkan Tabel~\ref{table:4.cross-dataset}, performa kedua model pada evaluasi lintas-\textit{dataset} lebih rendah dibandingkan performa pada \textit{dataset} \textit{WildDeepfake}. Kondisi ini menunjukkan bahwa perbedaan distribusi data antara \textit{WildDeepfake} dan \textit{Celeb-DF v2} memberikan tantangan terhadap kemampuan generalisasi model. Karena tidak dilakukan \textit{fine-tuning} pada \textit{Celeb-DF v2}, hasil E6-SP dan E6-ST merepresentasikan kemampuan transfer langsung dari representasi yang dipelajari pada \textit{WildDeepfake} ke \textit{dataset} eksternal.

Pada evaluasi ini, model spasial E6-SP memperoleh performa lebih tinggi dibandingkan model spasiotemporal E6-ST pada seluruh metrik evaluasi. Hasil tersebut menunjukkan bahwa representasi visual statis yang dipelajari oleh model spasial lebih mampu bertahan pada skenario lintas-\textit{dataset} dibandingkan representasi spasiotemporal dari model \textit{VideoMAE}. Hal ini dapat terjadi karena pola temporal yang dipelajari pada \textit{WildDeepfake} tidak sepenuhnya sesuai dengan karakteristik temporal pada \textit{Celeb-DF v2}, sehingga informasi temporal yang sebelumnya membantu pada \textit{dataset} sumber tidak selalu memberikan keuntungan yang sama pada \textit{dataset} eksternal.

Sebaliknya, model spasial hanya bergantung pada informasi visual \textit{frame-level}, sehingga representasi yang digunakan cenderung lebih sederhana dan tidak terlalu dipengaruhi oleh perbedaan dinamika antar-\textit{frame}. Meskipun demikian, nilai metrik E6-SP juga tetap mengalami penurunan dibandingkan hasil pada \textit{WildDeepfake}. Hal ini menunjukkan bahwa pendekatan spasial juga masih terdampak oleh perbedaan domain, seperti kualitas visual, karakteristik manipulasi, distribusi wajah, dan pola artefak pada \textit{dataset} yang berbeda.

Secara umum, evaluasi lintas-\textit{dataset} menunjukkan bahwa kemampuan generalisasi model masih menjadi tantangan penting dalam klasifikasi video \textit{deepfake}. Pada eksperimen utama \textit{WildDeepfake}, model spasiotemporal cenderung lebih unggul karena mampu memanfaatkan hubungan antar-\textit{frame}. Namun, pada evaluasi \textit{zero-shot} terhadap \textit{Celeb-DF v2}, model spasial justru menghasilkan performa yang lebih baik. Temuan ini menunjukkan bahwa keunggulan pendekatan spasiotemporal tidak selalu bersifat mutlak ketika model diuji pada domain data yang berbeda. Oleh karena itu, pemilihan pendekatan model perlu mempertimbangkan tidak hanya performa pada \textit{dataset} sumber, tetapi juga ketahanan model terhadap pergeseran distribusi data pada skenario lintas-\textit{dataset}.
