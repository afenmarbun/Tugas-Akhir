...
Pada evaluasi ini, \textit{checkpoint} model yang digunakan berasal dari hasil \textit{ablation study scheduler}, yaitu M6-SP untuk model spasial dan M6-ST untuk model spasiotemporal. Hasil pengujian pada \textit{Celeb-DF v2} diberi kode E6-SP untuk model spasial dan E6-ST untuk model spasiotemporal.
...
Berdasarkan Tabel~\ref{table:4.cross-dataset}, hasil evaluasi lintas-\textit{dataset} menunjukkan adanya pembalikan performa. Pada eksperimen internal menggunakan \textit{WildDeepfake}, model spasiotemporal secara umum menunjukkan performa yang lebih tinggi dibandingkan model spasial, khususnya pada eksperimen \textit{baseline} dan eksperimen utama. Namun, ketika model diuji secara \textit{zero-shot} pada \textit{Celeb-DF v2}, pola tersebut berubah. Model spasial E6-SP memperoleh \textit{accuracy} sebesar 72,9\% dan \textit{AUC} sebesar 77,7\%, sedangkan model spasiotemporal E6-ST memperoleh \textit{accuracy} sebesar 64,2\% dan \textit{AUC} sebesar 72,6\%. Dengan demikian, model spasial justru lebih unggul dibandingkan model spasiotemporal pada skenario lintas-\textit{dataset}.

Hal ini menunjukkan bahwa keunggulan model spasiotemporal pada \textit{dataset} sumber tidak secara otomatis menghasilkan generalisasi yang lebih baik pada \textit{dataset} eksternal. Pada \textit{dataset} \textit{WildDeepfake}, \textit{VideoMAE} memperoleh keuntungan karena mampu memodelkan hubungan spasial dan temporal antar-\textit{frame} melalui pemrosesan klip secara langsung. Akan tetapi, ketika model dipindahkan ke \textit{Celeb-DF v2} tanpa \textit{fine-tuning}, performa model spasiotemporal mengalami degradasi yang lebih besar. Jika dibandingkan dengan performa \textit{baseline} pada \textit{WildDeepfake}, \textit{accuracy} model spasiotemporal menurun dari 85,8\% menjadi 64,2\%, atau turun sebesar 21,6 poin persentase. Penurunan ini lebih besar dibandingkan model spasial yang turun dari 82,5\% pada M6-SP menjadi 72,9\% pada E6-SP, atau turun sebesar 9,6 poin persentase.

Salah satu penjelasan teoretis terhadap penurunan yang lebih besar pada model spasiotemporal adalah adanya pergeseran distribusi temporal atau \textit{temporal domain shift}. \textit{WildDeepfake} merupakan \textit{dataset in-the-wild} yang dikumpulkan dari video internet, sehingga memiliki variasi kondisi visual dan temporal yang tinggi, seperti perbedaan kualitas video, kompresi, \textit{frame rate}, pergerakan kamera, serta artefak temporal yang tidak seragam [9]. Dalam proses \textit{fine-tuning} pada \textit{dataset} tersebut, \textit{VideoMAE} dapat mempelajari pola dinamika temporal yang spesifik terhadap karakteristik \textit{WildDeepfake}. Melalui mekanisme \textit{joint space-time attention}, model tidak hanya mempelajari artefak visual pada \textit{frame}, tetapi juga relasi antar-\textit{frame} dalam satu klip. Ketika pola temporal pada \textit{Celeb-DF v2} berbeda dari \textit{WildDeepfake}, representasi temporal yang dipelajari model pada \textit{dataset} sumber dapat menjadi kurang relevan. Kondisi ini mengindikasikan bahwa model spasiotemporal berpotensi lebih rentan terhadap pergeseran domain temporal.

Sebaliknya, model spasial berbasis \textit{ViT} hanya memproses informasi visual statis pada tingkat \textit{frame}, kemudian melakukan agregasi prediksi pada tingkat klip. Karena tidak memodelkan hubungan temporal secara eksplisit, model spasial tidak terlalu bergantung pada pola gerakan, \textit{frame rate}, atau dinamika antar-\textit{frame} yang spesifik terhadap \textit{dataset} sumber. Representasi yang dipelajari model spasial lebih berfokus pada artefak visual \textit{frame-level}, seperti tekstur lokal, batas pencampuran wajah, ketidakwajaran detail visual, atau pola manipulasi statis lainnya. Fitur-fitur spasial tersebut cenderung lebih invarian terhadap perubahan domain temporal dibandingkan fitur spasiotemporal yang secara langsung melibatkan dimensi waktu. Hal ini dapat menjelaskan mengapa E6-SP memiliki penurunan performa yang lebih kecil dibandingkan E6-ST pada evaluasi \textit{Celeb-DF v2}.

Perbedaan karakteristik \textit{dataset} juga menjadi faktor penting dalam interpretasi hasil ini. \textit{WildDeepfake} merepresentasikan data \textit{in-the-wild} yang berasal dari internet, sedangkan \textit{Celeb-DF v2} dikenal sebagai \textit{dataset} \textit{deepfake} yang dirancang dengan kualitas visual yang lebih baik dan lebih menantang dibandingkan \textit{dataset} generasi awal [19]. Perbedaan sumber data, proses manipulasi, kualitas visual, dan karakteristik temporal dapat menyebabkan model yang telah beradaptasi dengan \textit{WildDeepfake} tidak sepenuhnya sesuai ketika diterapkan pada \textit{Celeb-DF v2}. Dengan demikian, penurunan performa pada E6-SP dan E6-ST menunjukkan adanya \textit{domain shift}, sedangkan pembalikan performa antara model spasial dan spasiotemporal menunjukkan bahwa fitur temporal yang efektif pada \textit{dataset} sumber belum tentu memiliki tingkat transferabilitas yang sama pada \textit{dataset} eksternal.

Secara keseluruhan, evaluasi lintas-\textit{dataset} menunjukkan bahwa kemampuan generalisasi model masih menjadi tantangan penting dalam klasifikasi video \textit{deepfake}. Pada eksperimen internal \textit{WildDeepfake}, model spasiotemporal lebih unggul karena mampu memanfaatkan hubungan antar-\textit{frame}. Namun, pada evaluasi \textit{zero-shot} terhadap \textit{Celeb-DF v2}, model spasial menunjukkan ketahanan yang lebih baik terhadap pergeseran domain. Temuan ini menunjukkan bahwa pendekatan spasiotemporal memiliki potensi kuat ketika distribusi data latih dan data uji relatif sejalan, tetapi dapat mengalami degradasi lebih besar ketika pola temporal pada \textit{dataset} uji berbeda. Oleh karena itu, pengembangan model \textit{deepfake} yang \textit{robust} tidak hanya perlu mengejar performa tinggi pada \textit{dataset} sumber, tetapi juga perlu mempertimbangkan kemampuan generalisasi lintas-\textit{dataset}.
