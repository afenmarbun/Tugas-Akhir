...
\begin{enumerate}[noitemsep]
	...
	\item Model yang digunakan dibatasi pada arsitektur Vision Transformer (ViT) untuk pendekatan spasial dan encoder VideoMAE untuk pendekatan spasiotemporal, dengan memanfaatkan bobot pralatih (\textit{pretrained weights}) untuk proses \textit{fine-tuning}.
...
\begin{enumerate}[noitemsep]
	\item Menghasilkan implementasi dan evaluasi komparatif model klasifikasi video \textit{deepfake} menggunakan pendekatan spasial berbasis Vision Transformer (ViT) dan pendekatan spasiotemporal berbasis encoder VideoMAE pada dataset WildDeepfake.
