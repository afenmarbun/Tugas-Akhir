...
Pada tahap ini, setiap video wajah pada \textit{dataset} \textit{WildDeepfake} diproses menjadi klip dengan panjang tetap 46 \textit{frame}. Pembentukan klip dilakukan agar seluruh data memiliki panjang temporal yang seragam sebelum digunakan pada proses \textit{strided temporal sampling}. Dengan demikian, unit data yang digunakan pada tahap eksperimen bukan lagi video wajah awal, melainkan klip 46 \textit{frame}.
...
Pembentukan klip 46 \textit{frame} juga bertujuan agar \textit{frame} pada video yang lebih panjang tidak terlalu banyak terbuang. Karena model pada penelitian ini hanya membutuhkan 16 \textit{frame} sebagai masukan untuk satu klip, satu video yang memiliki ratusan \textit{frame} dapat dipecah menjadi beberapa klip sebelum dilakukan \textit{strided temporal sampling}. Dengan strategi ini, informasi temporal dari satu video dapat dimanfaatkan secara lebih efisien dan jumlah data eksperimen yang dihasilkan menjadi lebih banyak.

Berdasarkan hasil pembentukan klip 46 \textit{frame}, diperoleh total 22.303 klip. Seluruh klip tersebut memiliki panjang 46 \textit{frame}, sehingga tidak terdapat klip yang tidak valid. Distribusi kelas hasil pembentukan klip 46 \textit{frame} ditunjukkan pada Tabel~\ref{table:4.klip46}.
...
# agar klip turunan seperti 101_0, 101_1, dan 101_2 tetap berada pada subset yang sama.
group_df = (
...
Kode~\ref{code:4.4} menunjukkan bahwa citra masukan terlebih dahulu dikonversi menjadi \textit{tensor} dengan format C $\times$ H $\times$ W. Selanjutnya, nilai piksel dikonversi ke tipe \textit{float32} dan diskalakan ke rentang [0, 1]. Setelah itu, nilai piksel dibatasi pada rentang [0, 1] menggunakan \texttt{torch.clamp}, kemudian dinormalisasi menggunakan nilai mean (0,485, 0,456, 0,406) dan standard deviation (0,229, 0,224, 0,225). Proses normalisasi ini dilakukan agar distribusi nilai piksel sesuai dengan karakteristik model pralatih yang digunakan.
...
\begin{figure}[htbp]
	\centering
	\borderedincludegraphics[width=0.65\textwidth]{\detokenize{figure/Hasil Eksperimen/Eksperimen Utama 1 Baseline/Model Spasial/Loss.png}}
	\caption{Kurva \textit{training loss} dan \textit{validation loss} pada eksperimen \textit{baseline} untuk model spasial.}
	\label{fig:4.baseline-loss-spatial}
\end{figure}

\begin{figure}[htbp]
	\centering
	\borderedincludegraphics[width=0.65\textwidth]{\detokenize{figure/Hasil Eksperimen/Eksperimen Utama 1 Baseline/Model Spasiotemporal/Loss.png}}
	\caption{Kurva \textit{training loss} dan \textit{validation loss} pada eksperimen \textit{baseline} untuk model spasiotemporal.}
	\label{fig:4.baseline-loss-spatiotemporal}
\end{figure}

Berdasarkan Gambar~\ref{fig:4.baseline-loss-spatial} dan Gambar~\ref{fig:4.baseline-loss-spatiotemporal}, kurva \textit{training loss} pada model spasial dan spasiotemporal sama-sama menurun secara bertahap selama proses pelatihan. Pola ini menunjukkan bahwa kedua model mampu mempelajari pola dari data latih dengan baik. Sementara itu, \textit{validation loss} pada kedua model masih menunjukkan fluktuasi pada beberapa \textit{epoch}, tetapi tidak mengalami peningkatan secara terus-menerus. Hal ini mengindikasikan bahwa proses pelatihan masih berjalan stabil dan tidak menunjukkan gejala \textit{overfitting} yang berat pada konfigurasi \textit{baseline}.
...
\begin{figure}[htbp]
	\centering
	\borderedincludegraphics[width=0.65\textwidth]{\detokenize{figure/Hasil Eksperimen/Eksperimen Utama 1 Baseline/Model Spasial/Accuracy.png}}
	\caption{Kurva \textit{training accuracy} dan \textit{validation accuracy} pada eksperimen \textit{baseline} untuk model spasial.}
	\label{fig:4.baseline-acc-spatial}
\end{figure}

Berdasarkan Gambar~\ref{fig:4.baseline-acc-spatial} dan Gambar~\ref{fig:4.baseline-acc-spatiotemporal}, kurva \textit{training accuracy} pada kedua model menunjukkan peningkatan secara bertahap dari awal hingga akhir pelatihan. \textit{validation accuracy} juga memperlihatkan tren peningkatan dan cenderung stabil pada \textit{epoch} akhir. Hal ini menunjukkan bahwa kedua model mampu meningkatkan kemampuan klasifikasi selama proses pelatihan.
...
\begin{figure}[htbp]
	\centering
	\borderedincludegraphics[width=0.65\textwidth]{\detokenize{figure/Hasil Eksperimen/Eksperimen Utama 1 Baseline/Model Spasiotemporal/Accuracy.png}}
	\caption{Kurva \textit{training accuracy} dan \textit{validation accuracy} pada eksperimen \textit{baseline} untuk model spasiotemporal.}
	\label{fig:4.baseline-acc-spatiotemporal}
\end{figure}
...
\begin{figure}[htbp]
	...
	\begin{minipage}{0.48\textwidth}
		\centering
		\borderedincludegraphics[width=\textwidth]{\detokenize{figure/Hasil Eksperimen/Eksperimen Utama 1 Baseline/Model Spasial/confusion-matrix.png}}
		\\(a) Model spasial
	\end{minipage}
	\hfill
	\begin{minipage}{0.48\textwidth}
		\centering
		\borderedincludegraphics[width=\textwidth]{\detokenize{figure/Hasil Eksperimen/Eksperimen Utama 1 Baseline/Model Spasiotemporal/confusion-matrix.png}}
...
Untuk mendukung hasil tersebut, Gambar~\ref{fig:4.ablation-epoch-loss-spatial} dan Gambar~\ref{fig:4.ablation-epoch-loss-spatiotemporal} menampilkan kurva \textit{training loss} dan \textit{validation loss} pada konfigurasi 40 \textit{epoch} untuk model spasial dan spasiotemporal.

\begin{figure}[htbp]
\centering
\borderedincludegraphics[width=0.75\textwidth]{\detokenize{figure/Hasil Eksperimen/Ablation Study/M01/Model Spasial/Loss.png}}
\caption{Kurva \textit{training loss} dan \textit{validation loss} pada eksperimen \textit{epoch} maksimum 40 untuk model spasial.}
\label{fig:4.ablation-epoch-loss-spatial}
\end{figure}

Secara umum, \textit{training loss} pada kedua model terus menurun hingga akhir pelatihan. Pola tersebut menunjukkan bahwa model semakin mampu menyesuaikan bobotnya terhadap data latih seiring bertambahnya jumlah \textit{epoch}. Namun, penurunan \textit{training loss} tidak diikuti oleh penurunan \textit{validation loss} yang konsisten. Sebaliknya, \textit{validation loss} pada kedua model cenderung berfluktuasi pada \textit{epoch}-\textit{epoch} lanjutan. Kondisi ini menunjukkan bahwa tambahan \textit{epoch} tidak lagi memberikan perbaikan yang stabil pada data validasi.

\begin{figure}[htbp]
\centering
\borderedincludegraphics[width=0.75\textwidth]{\detokenize{figure/Hasil Eksperimen/Ablation Study/M01/Model Spasiotemporal/Loss.png}}
\caption{Kurva \textit{training loss} dan \textit{validation loss} pada eksperimen \textit{epoch} maksimum 40 untuk model spasiotemporal.}
\label{fig:4.ablation-epoch-loss-spatiotemporal}
\end{figure}

Pada model spasial, fluktuasi \textit{validation loss} mulai terlihat setelah pertengahan pelatihan dan bertahan hingga akhir \textit{epoch}. Sementara itu, pada model spasiotemporal, \textit{validation loss} juga menunjukkan pola yang tidak stabil, bahkan beberapa kali meningkat meskipun \textit{training loss} terus menurun. Pola \textit{training loss} yang terus menurun sementara \textit{validation loss} tidak membaik secara konsisten mengindikasikan adanya kecenderungan \textit{overfitting}. Dalam kondisi tersebut, model semakin menyesuaikan diri terhadap data latih, tetapi penyesuaian tersebut tidak diikuti oleh peningkatan kemampuan generalisasi pada data validasi.

Indikasi \textit{overfitting} tersebut juga sejalan dengan hasil evaluasi pada Tabel~\ref{table:4.ablation-epoch}. Penambahan \textit{epoch} maksimum dari 25 menjadi 40 tidak meningkatkan performa kedua pendekatan pada data uji. Pada model spasial, \textit{accuracy} menurun dari 79,2\% menjadi 78,0\%, sedangkan \textit{F1-score} menurun dari 76,7\% menjadi 76,3\%. Pada model spasiotemporal, \textit{accuracy} menurun dari 85,8\% menjadi 84,5\%, sedangkan \textit{F1-score} menurun dari 84,5\% menjadi 83,2\%. Dengan demikian, penambahan durasi pelatihan membuat model lebih lama menyesuaikan diri terhadap data latih, tetapi tidak menghasilkan peningkatan generalisasi.

Berdasarkan hasil tersebut, konfigurasi \textit{epoch} maksimum 40 tidak dipilih sebagai konfigurasi lanjutan. Konfigurasi \textit{baseline} dengan 25 \textit{epoch} dinilai lebih memadai karena menghasilkan performa yang lebih baik dan proses pelatihan yang lebih efisien. Temuan adanya kecenderungan \textit{overfitting} pada eksperimen \textit{epoch} maksimum ini juga menjadi dasar untuk menganalisis konfigurasi regularisasi pada eksperimen berikutnya, yaitu penggunaan \textit{dropout}.
...
\begin{figure}[htbp]
\centering
\borderedincludegraphics[width=0.65\textwidth]{\detokenize{figure/Hasil Eksperimen/Ablation Study/M02/Model Spasial/Loss.png}}
\caption{Kurva \textit{training loss} dan \textit{validation loss} pada eksperimen \textit{learning rate} 5e-4 untuk model spasial.}
\label{fig:4.ablation-lr-loss-spatial}
\end{figure}

Pola pada Gambar~\ref{fig:4.ablation-lr-loss-spatial} dan Gambar~\ref{fig:4.ablation-lr-loss-spatiotemporal} memperkuat temuan tersebut. Pada kedua model, \textit{training loss} terus menurun selama proses pelatihan, tetapi \textit{validation loss} tidak mengikuti pola penurunan yang sama. \textit{Validation loss} justru cenderung berfluktuasi dan berada pada nilai yang relatif tinggi sepanjang pelatihan. Pola ini menunjukkan bahwa model masih mampu menyesuaikan diri terhadap data latih, tetapi penyesuaian tersebut tidak menghasilkan peningkatan kemampuan generalisasi pada data validasi. Dengan demikian, \textit{learning rate} 5e-4 membuat proses \textit{fine-tuning} menjadi tidak stabil dan menyebabkan jarak antara performa pelatihan dan validasi semakin jelas.

\begin{figure}[htbp]
\centering
\borderedincludegraphics[width=0.65\textwidth]{\detokenize{figure/Hasil Eksperimen/Ablation Study/M02/Model Spasiotemporal/Loss.png}}
\caption{Kurva \textit{training loss} dan \textit{validation loss} pada eksperimen \textit{learning rate} 5e-4 untuk model spasiotemporal.}
\label{fig:4.ablation-lr-loss-spatiotemporal}
\end{figure}
...
Selain itu, \textit{Focal Loss} juga relevan digunakan pada kondisi distribusi data yang tidak seimbang. Pada \textit{dataset} hasil pembentukan klip, jumlah klip kelas \textit{fake} lebih banyak dibandingkan kelas \textit{real}, sehingga model berpotensi lebih mudah mempelajari pola dari kelas yang lebih dominan. \textit{Focal Loss} dirancang untuk mengurangi kontribusi sampel yang mudah diklasifikasikan dan memberikan perhatian lebih besar pada sampel yang sulit, sehingga fungsi \textit{loss} ini dapat membantu model dalam menangani ketidakseimbangan data dan kesalahan klasifikasi pada kelas yang lebih sulit dikenali \cite{lin2018focallossdenseobject}.

Pada implementasi penelitian ini, \textit{Focal Loss} tidak hanya menggunakan faktor gamma untuk menekankan sampel yang sulit diklasifikasikan, tetapi juga menggunakan \textit{alpha class weighting} untuk memberikan bobot berbeda pada setiap kelas. Nilai alpha dihitung berdasarkan distribusi kelas pada data latih dengan prinsip bobot berbanding terbalik terhadap jumlah sampel tiap kelas. Berdasarkan Tabel~\ref{table:4.split}, data latih terdiri atas 5.376 klip \textit{real} dan 10.124 klip \textit{fake}. Karena jumlah klip \textit{real} lebih sedikit dibandingkan klip \textit{fake}, kelas \textit{real} memperoleh bobot alpha yang lebih besar dibandingkan kelas \textit{fake}. Dengan distribusi tersebut, bobot alpha ternormalisasi yang digunakan adalah sekitar 1,306 untuk kelas \textit{real} dan 0,694 untuk kelas \textit{fake}. Artinya, kesalahan pada kelas \textit{real} memperoleh bobot sekitar 1,88 kali lebih besar dibandingkan kesalahan pada kelas \textit{fake}. Dengan demikian, penerapan \textit{Focal Loss} pada penelitian ini secara eksplisit memberikan perhatian lebih besar pada kelas \textit{real} atau normal sebagai kelas minoritas, sedangkan faktor gamma tetap berfungsi untuk menekankan sampel yang sulit diklasifikasikan dari kedua kelas.
...
\begin{figure}[htbp]
\centering
\borderedincludegraphics[width=0.9\textwidth]{\detokenize{figure/Hasil Eksperimen/Ablation Study/M03/Perbandingan Confusion Matrix - Model Spasial.jpg}}
...
\begin{figure}[htbp]
\centering
\borderedincludegraphics[width=0.9\textwidth]{\detokenize{figure/Hasil Eksperimen/Ablation Study/M03/Perbandingan Confusion Matrix - Model Spasiotemporal.jpg}}
...
Jika hasil \textit{confusion matrix} dihubungkan dengan mekanisme \textit{alpha class weighting}, pola penurunan \textit{false positive} dapat dijelaskan oleh bobot kelas \textit{real} yang lebih besar. Karena kelas \textit{real} merupakan kelas minoritas pada data latih, kesalahan terhadap kelas \textit{real} diberi penalti lebih besar selama proses pelatihan. Akibatnya, model menjadi lebih berhati-hati dalam mengklasifikasikan klip \textit{real} sebagai \textit{fake}. Hal ini terlihat dari penurunan \textit{false positive} pada model spasial maupun spasiotemporal setelah menggunakan \textit{Focal Loss}. Dengan kata lain, penggunaan \textit{Focal Loss} dengan \textit{alpha class weighting} pada penelitian ini lebih mengutamakan penanganan kesulitan klasifikasi pada kelas \textit{real} atau normal sebagai kelas minoritas, bukan pada kelas \textit{fake} yang jumlahnya lebih dominan setelah pembentukan klip 46 \textit{frame}.

Namun, peningkatan perhatian terhadap kelas \textit{real} tersebut memiliki konsekuensi terhadap kemampuan model dalam mendeteksi kelas \textit{fake}. Berdasarkan Gambar~\ref{fig:4.ablation-loss-cm-spatial} dan Gambar~\ref{fig:4.ablation-loss-cm-spatiotemporal}, penurunan \textit{false positive} disertai dengan peningkatan \textit{false negative}. Artinya, sebagian klip \textit{fake} menjadi lebih banyak yang salah diprediksi sebagai \textit{real}. Pada model spasial, perubahan ini masih menghasilkan peningkatan performa keseluruhan, sebagaimana ditunjukkan oleh peningkatan \textit{accuracy}, \textit{F1-score}, dan AUC pada Tabel~\ref{table:4.ablation-loss}. Sebaliknya, pada model spasiotemporal, peningkatan \textit{false negative} lebih besar sehingga performa akhirnya menurun dibandingkan \textit{baseline Cross Entropy Loss}. Dengan demikian, \textit{Focal Loss} dengan \textit{alpha class weighting} efektif untuk menekan kesalahan prediksi \textit{real} sebagai \textit{fake}, tetapi belum optimal untuk meningkatkan sensitivitas deteksi terhadap seluruh klip \textit{fake}.
...
Eksperimen \textit{dropout} dilakukan untuk mengevaluasi apakah regularisasi tambahan dapat membantu mengurangi kecenderungan \textit{overfitting} yang terlihat pada eksperimen \textit{epoch} maksimum. Pada \textit{baseline}, nilai \textit{dropout} yang digunakan adalah 0, sedangkan pada eksperimen ini nilai \textit{dropout} dinaikkan menjadi 0,1. \textit{Dropout} merupakan salah satu teknik regularisasi yang bekerja dengan menonaktifkan sebagian unit secara acak selama proses pelatihan. Mekanisme ini bertujuan untuk mengurangi ketergantungan model terhadap kombinasi fitur tertentu pada data latih, sehingga dapat membantu meningkatkan kemampuan generalisasi model \cite{JMLR:v15:srivastava14a}.

Berdasarkan Tabel~\ref{table:4.ablation-dropout}, penggunaan \textit{dropout} 0,1 memberikan dampak yang berbeda pada model spasial dan spasiotemporal. Pada model spasial, \textit{dropout} memberikan peningkatan kecil pada seluruh metrik evaluasi dibandingkan \textit{baseline}. \textit{Accuracy} meningkat dari 79,2\% menjadi 79,4\%, \textit{F1-score} meningkat dari 76,7\% menjadi 77,4\%, dan \textit{AUC} meningkat dari 86,8\% menjadi 87,1\%. Peningkatan ini menunjukkan bahwa regularisasi tambahan dapat membantu model spasial mengurangi ketergantungan terhadap pola visual tertentu pada data latih. Dengan demikian, \textit{dropout} memberikan manfaat terbatas pada pendekatan spasial, terutama dalam konteks pengendalian \textit{overfitting}.

Namun, pola berbeda terlihat pada model spasiotemporal. Pada model \textit{VideoMAE}, penggunaan \textit{dropout} 0,1 justru menurunkan performa dibandingkan \textit{baseline}. \textit{Accuracy} menurun dari 85,8\% menjadi 83,8\%, \textit{F1-score} menurun dari 84,5\% menjadi 82,8\%, dan \textit{AUC} menurun dari 92,3\% menjadi 91,0\%. Hasil ini menunjukkan bahwa regularisasi tambahan tidak selalu memberikan dampak positif pada model spasiotemporal. Salah satu kemungkinan penyebabnya adalah model \textit{VideoMAE} telah memiliki representasi awal yang kuat dari proses \textit{pre-training} pada domain video, sehingga penambahan \textit{dropout} dapat mengganggu pemanfaatan representasi spasiotemporal yang telah dipelajari sebelumnya selama proses \textit{fine-tuning}.

Jika dihubungkan dengan eksperimen \textit{epoch} maksimum, \textit{dropout} dapat dipahami sebagai upaya untuk mengendalikan kecenderungan \textit{overfitting} yang muncul ketika durasi pelatihan diperpanjang. Akan tetapi, hasil eksperimen menunjukkan bahwa efektivitas \textit{dropout} tidak seragam pada kedua pendekatan. \textit{Dropout} 0,1 memberikan perbaikan kecil pada model spasial, tetapi menurunkan performa model spasiotemporal. Dengan demikian, regularisasi model perlu disesuaikan dengan karakteristik arsitektur dan representasi awal model. Konfigurasi \textit{dropout} tidak dipilih sebagai konfigurasi utama karena manfaatnya tidak konsisten pada kedua pendekatan.
...
Eksperimen penurunan resolusi dilakukan untuk mengevaluasi pengaruh berkurangnya informasi spasial terhadap performa model. Pada konfigurasi ini, resolusi masukan diturunkan menjadi 112 $\times$ 112 piksel. Pengujian ini penting karena data video dunia nyata tidak selalu berada dalam kualitas visual yang ideal. Oleh karena itu, eksperimen ini digunakan untuk melihat sejauh mana model spasial dan spasiotemporal tetap mampu melakukan klasifikasi ketika informasi visual pada \textit{frame} menjadi lebih terbatas.
...
\midrule
BASE-ST & \textbf{85,8\%} & \textbf{83,9\%} & 85,5\% & \textbf{84,5\%} & 92,3\% \\
E4-ST & 85,0\% & 83,5\% & \textbf{86,7\%} & 84,2\% & \textbf{92,4\%} \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
BASE-ST & \textbf{85,8\%} & \textbf{83,9\%} & 85,5\% & \textbf{84,5\%} & 92,3\% \\
E5-ST & 85,1\% & 83,3\% & \textbf{86,3\%} & 84,1\% & \textbf{92,7\%} \\
\bottomrule
\end{tabular}
\end{table}
